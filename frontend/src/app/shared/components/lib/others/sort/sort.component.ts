...
export class SortComponent  implements OnInit {
  ...
  constructor()  {}
  ngOnInit(): void {}
  ngOnChanges(changes: SimpleChanges): void {this.sort_by_name_value = 'sort'}
  ...
  sortByNameFunction(): any {
      ...
      if (this.sort_by_name_value == 'sort-up') {
        this.data.sort((a: any, b: any) => a[this.path].toLowerCase() > b[this.path].toLowerCase() ? 1 : -1)
  ...
  sortByDateFunction(i: string) {
    if (i == 'sort-down') {
      ...
      this.data.sort((a: any, b: any) => (a[this.path] > b[this.path] ? 1 : -1))
    } else if (i == 'sort-up') {
